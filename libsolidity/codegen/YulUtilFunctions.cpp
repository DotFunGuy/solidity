	...
	return m_functionCollector.createFunction(functionName, [&](std::vector<std::string>& _args, std::vector<std::string>&) {
		...
		return Whiskers(R"(
			if lt(startIndex, len) {
				...
				<clearStorageRange>(deleteStart, <convertToSize>(sub(len, startIndex)))
			}
		)")
		...
		return Whiskers(R"(
			if gt(len, 31) {
				if gt(len, startIndex) {
					...
					let numItemsToClear := sub(len, startIndex)
					// If we are clearing array to be short byte array, we want to clear only data starting from array data area.
					if lt(startIndex, 32) {
						deleteStart := dataArea
						numItemsToClear := len
					}
					<clearStorageRange>(deleteStart, <div32Ceil>(numItemsToClear))
				}
			}
		)")
		...
		return Whiskers(R"(
			function <functionName>(array, data, oldLen, newLen) {
				...
				case  0 {
					...
					<clearStorageRange>(deleteStart, <div32Ceil>(sub(oldLen, newLen)))
				...
				default {
					...
					case 1 {
						...
						<clearStorageRange>(add(arrayDataStart, 1), <div32Ceil>(oldLen))
...
std::string YulUtilFunctions::clearStorageRangeFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		return Whiskers(R"(
			function <functionName>(start, slotCount) {
				for { let i := 0 } lt(i, slotCount) { i := add(i, <increment>) }
				{
					<setToZero>(add(start, i), 0)
				}
			}
		)")
		("functionName", functionName)
